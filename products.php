        ...
        if ($action === 'add_product') {
            $stmt = $conn->prepare("INSERT INTO products
                (name,barcode,category_id,brand,description,cost_price,selling_price,
                 ...
                 min_stock_level,max_stock_level,reorder_level,is_active,is_featured)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->bind_param('ssissddddssiiiis i',
                $name,$barcode,$cat_id,$brand,$description,$cost,$sell,
                $wholesale,$uom,$upc,$weight,$expiry,
                $min_stock,$max_stock,$reorder,$is_active,$is_featured);
            $stmt->close();
            // Re-prepare cleanly
            $stmt = $conn->prepare("INSERT INTO products
                (name,barcode,category_id,brand,description,cost_price,selling_price,
                 ...
                 min_stock_level,max_stock_level,reorder_level,is_active,is_featured)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $stmt->bind_param('ssissdddsidsiiii',
                $name,$barcode,$cat_id,$brand,$description,$cost,$sell,
                $wholesale,$uom,$upc,$weight,$expiry,
                $min_stock,$max_stock,$reorder,$is_active,$is_featured);
            if ($stmt->execute()) {
            ...
            if ($pid) {
                ...
                $stmt->bind_param('ssissdddsidsiiii i',
                    $name,$barcode,$cat_id,$brand,$description,$cost,$sell,
                    $wholesale,$uom,$upc,$weight,$expiry,
                    $min_stock,$max_stock,$reorder,$is_active,$is_featured,$pid);
                $stmt->close();
                $stmt = $conn->prepare("UPDATE products SET
                    name=?,barcode=?,category_id=?,brand=?,description=?,
                    cost_price=?,selling_price=?,wholesale_price=?,
                    ...
                    min_stock_level=?,max_stock_level=?,reorder_level=?,
                    is_active=?,is_featured=? WHERE id=?");
                $stmt->bind_param('ssissdddsiidsiiii i',
                    $name,$barcode,$cat_id,$brand,$description,$cost,$sell,
                    $wholesale,$uom,$upc,$weight,$expiry,
                    $min_stock,$max_stock,$reorder,$is_active,$is_featured,$pid);
                $stmt->close();

                // use query directly to avoid bind complexity
                $n  = esc($conn,$name);
                $bc = $barcode ? "'".esc($conn,$barcode)."'" : 'NULL';
                $ci = $cat_id  ? $cat_id : 'NULL';
                $br = $brand   ? "'".esc($conn,$brand)."'" : 'NULL';
                $ds = $description ? "'".esc($conn,$description)."'" : 'NULL';
                $wh = $wholesale   ? $wholesale : 'NULL';
                $wt = $weight      ? $weight : 'NULL';
                $ex = $expiry      ? "'$expiry'" : 'NULL';

                $sql = "UPDATE products SET
                    name='$n', barcode=$bc, category_id=$ci, brand=$br,
                    description=$ds, cost_price=$cost, selling_price=$sell,
                    wholesale_price=$wh, unit_of_measure='$uom', units_per_case=$upc,
                    weight_kg=$wt, expiry_date=$ex,
                    min_stock_level=$min_stock, max_stock_level=$max_stock,
                    reorder_level=$reorder, is_active=$is_active, is_featured=$is_featured
                    WHERE id=$pid";
                if ($conn->query($sql)) {
                    $conn->query("INSERT INTO audit_logs (user_id,user_name,action,table_name,record_id,description)
...
.prod-thumb{width:36px;height:36px;border-radius:9px;background:var(--bg3);border:1px solid var(--br);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:.8rem;font-weight:800;color:var(--t2)}
...
.modal{background:var(--bg2);border:1px solid var(--br2);border-radius:18px;width:100%;max-width:640px;max-height:90vh;overflow-y:auto;box-shadow:var(--sh2);transform:translateY(20px) scale(.97);transition:transform .28s cubic-bezier(.16,1,.3,1)}
...
<div class="main">
  ...
  <div class="topnav">
    ...
    <div class="bc">
      ...
      <span>Inventory</span><span class="sep">/</span>
      <span class="cur">Products &amp; Categories</span>
    </div>
    <div class="tnr">
      <div class="sbar">
      ...
      <a href="notifications.php" class="ibtn">
  ...
  <div class="content">
    ...
    <!-- ═══════════════════════════════════════════
         PRODUCTS TAB  —  continues in Part 2
    ═══════════════════════════════════════════ -->
    <?php if ($tab === 'products'): ?>
      ...
      <div style="overflow-x:auto">
        <table class="dtbl">
          ...
          <tr>
            <td>
              <div class="prod-cell">
                <div class="prod-thumb"><?= strtoupper(substr($prod['name'],0,2)) ?></div>
                <div>
